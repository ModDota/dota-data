...
import { getDotaDirectoryContents, getDotaVdfFile, outputFile, outputJson } from './utils';
...
export async function generateLocalization({ languages }: { languages?: string[] } = {}) {
  ...
  await Promise.all([
    outputFile('index.js', index(fetchedLanguages)),
    outputFile('index.d.ts', indexTypes(fetchedLanguages)),
    ...(languages != null ? languages : fetchedLanguages).map(async language => {
      ...
      await outputJson(`${language}`, result);
      await outputFile(
        `${language}.d.ts`,
